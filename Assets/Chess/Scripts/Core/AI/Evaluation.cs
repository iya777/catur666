...
public class Evaluation {
	...
	public const float endgameMaterialStart = rookValue * 2 + bishopValue + knightValue;
    public Board board;
	...
	public float EndgamePhaseWeight (int materialCountWithoutPawns) {
		const float multiplier = 1 / endgameMaterialStart;
		return 1 - System.Math.Min (1, materialCountWithoutPawns * multiplier);
	}
    public int MopUpEval (int friendlyIndex, int opponentIndex, int myMaterial, int opponentMaterial, float endgameWeight) {
		int mopUpScore = 0;
    ...
    public int EvaluatePieceSquareTables (int colourIndex, float endgamePhaseWeight) {
		int value = 0;
